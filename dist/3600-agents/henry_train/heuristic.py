...
W_POINT = float(os.getenv("W_POINT", 10000.0))
W_REACH = float(os.getenv("W_REACH", 100.0))
W_TERR = float(os.getenv("W_TERR", 10.0))
W_RAT = float(os.getenv("W_RAT", 5.0))
...
def evaluate_board(board: Board, is_player_a: bool, rat_belief: np.ndarray = None) -> float:
    ...
    score = point_diff * W_POINT + reachability_diff * W_REACH + territory_diff * W_TERR + rat_score * W_RAT
